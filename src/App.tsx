...
interface SquareProps {
    value: string | null; // O valor que o quadrado irá exibir: 'X', 'O' ou null
...
interface BoardProps {
    ...
    onPlay: (nextSquares: (string | null)[]) => void; // Função para notificar o Game sobre uma nova jogada.
}

// --- Componente Board ---
// Gerencia a exibição do tabuleiro e a lógica de clique nos quadrados, mas o estado é gerenciado pelo Game.
export function Board({ xIsNext, squares, onPlay }: BoardProps) {

    // Função que lida com o clique em qualquer um dos quadrados no indice "i" do quadrado clicado.
    function handleClick(i: number) {
        // Verifica se o jogo já acabou Ou se o quadrado clicado já está preenchido.
        if (calculateWinner(squares) || squares[i]) {
            return; // Se sim, não faz nada.
        }

        // Cria uma CÓPIA do array de squares.
        const nextSquares = squares.slice();

        // Define o valor do quadrado clicado ("X" ou "O") com base no xIsNext que veio via props.
        if (xIsNext) {
            nextSquares[i] = "X";
        } else {
            nextSquares[i] = "O";
        ...
        // Chama a função onPlay para notificar o componente pai (Game) sobre a nova jogada, passando o array nextSquares atualizado.
        onPlay(nextSquares);
    }
...
// Esta função apenas recebe um array de quadrados e verifica se alguém venceu, sem modificar nada ou depender de estado.
...
export default function Game() {
    const [history, setHistory] = useState<(string | null)[][]>([Array(9).fill(null)]); // history é um array de arrays, onde cada sub-array é um estado do tabuleiro em um determinado turno.
    const [currentMove, setCurrentMove] = useState(0);  // currentMove indica qual o índice da jogada atual no histórico.
    const xIsNext = currentMove % 2 === 0; // Determina se é a vez do 'X' (se o número da jogada atual for par).
    const currentSquares = history[currentMove]; // Pega o estado do tabuleiro na jogada atual do histórico.
